...
- Current collection: 180 pairs
- Next planned dataset: 2,000 draft pairs

This is the active specification for future creation. Editing this guide does not regenerate or retrospectively relabel existing datasets. The planned 2,000 pairs have not been generated by this documentation update.
...
Scenario weighting applies across categories, error combinations, and lengths, including already-correct examples. Domain is separate from primary error category and communication format: a technical email is still a technical scenario and may have email formatting as its primary category. Do not double-count it across scenario groups. This guidance does not rebalance or rewrite the existing 180 pairs.
...
The record-type composition remains 50% `single_principal_error`, 45% `natural_multi_error`, and 5% `no_change`. For 2,000 pairs, that is 1,000 / 900 / 100. A single-principal-error example centers on one behavior; multi-error examples normally combine two or three meaningful behaviors. Do not overload examples to fill quotas.

The following is the agreed principal-category allocation for the planned 2,000-pair batch, not a claim about measured production frequencies:

| Principal category | Planned pairs |
|---|---:|
| Fillers | 300 |
| Repetition and stutters | 300 |
| Brief false starts and immediate corrections | 200 |
| Punctuation, capitalization, dictated formatting | 300 |
| List formatting | 200 |
| Email formatting | 250 |
| Entity normalization | 250 |
| Context-inferred quotation | 100 |
| Already correct | 100 |
| **Total** | **2,000** |

These are two views of the same 2,000 pairs, not separate pools. Do not rebalance the existing 180 merely to fit future quotas.

Every newly created raw ASR input must contain at least 20 words, across every category, including no-change examples. Count words as non-empty whitespace-separated tokens in the user message, not in the system instruction or metadata. There is no minimum word count for the cleaned output. Do not pad inputs with meaningless filler, duplicate ideas, or unsupported facts to reach the minimum; author a naturally complete message instead. This rule applies from now on and does not require rewriting existing records.

Not everything should stop at 20 words. Mix shorter inputs of 20+ words, multi-sentence messages, and extended explanations. Retain at least 20% inputs of 30 words or more within sufficiently populated changed-record groups, spread across categories. The proposed extended subset is about 200 inputs of roughly 80–200 words, included within the 2,000 and the 30+ word coverage, not added on top. The extended-length allocation remains provisional.
...
Short examples in this section illustrate local editing rules, not complete dataset records. All newly authored dataset inputs must still meet the 20-word minimum.
...
4. Run quick mechanical checks: valid JSONL, roles/fields, count, unique IDs, duplicate inputs, at least 20 words in every newly generated raw input, category totals, no-change equality, and matching viewer copies.
5. Deliver to the viewer for the user's content review. Do not automatically run another record-by-record semantic audit or repeated review passes unless asked.
...
| Authoring file | IDs | Public viewer copy |
|---|---|---|
| `data/curated-180.jsonl` | `sample_001`–`sample_180` | `public/data/curated-180.jsonl` |
| `data/sample-50.jsonl` | 001–050 | `public/data/generator-sample.jsonl` |
| `data/sample-30.jsonl` | 051–080 | `public/data/additional-sample.jsonl` |
| `data/sample-50-additional.jsonl` | 081–180 | `public/data/additional-50-sample.jsonl` |

Public paths are relative to the repository root; authoring paths are relative to this directory. Historical filenames are retained. Edits to a component record must be reflected in the combined file and both public copies. Benchmark artifacts stay separate and retain their own IDs. The existing 180 pairs have not been rebalanced to future quotas.
