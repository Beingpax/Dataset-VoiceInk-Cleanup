...
export default function BenchmarkPage() {
  const { data, loading, error, dataset, setDataset, hiddenModelIds, setHiddenModelIds } = useBenchmark();
  ...
  const rankedModels = visibleModels.filter(model => isRanked(model, summaryFor(model)));
  ...
  return (
    <div className="page-stack">
      ...
      {!visibleModels.length ? <section className="ranking-section" aria-label="Benchmark results"><p className="empty-copy">No models selected. Turn on a model above to see its benchmark results.</p><button type="button" onClick={() => setHiddenModelIds(new Set())}>Show all models</button></section> : <>
      <section className="ranking-section" aria-label="System ranking">
        ...
        <div className="table-scroll"><table className="ranking-table" aria-describedby="ranking-scope"><thead><tr><th>Rank / configuration</th><th>Completed / expected</th><th>Failed / missing</th><th>Edit similarity</th><th>chrF++</th><th>WER</th><th>Exact / expected</th><th>Mean latency</th><th>Median tok/s</th><th>Peak memory</th></tr></thead><tbody>{visibleModels.map(model => {
      ...
      <section className="plot-section" aria-labelledby="quality-field-heading"><div className="section-title quality-section-title"><h2 id="quality-field-heading">Quality field</h2><span className="quality-direction">Better <span aria-hidden="true">↗</span></span></div>{rankedModels.length ? <QualityPlot key={dataset} models={rankedModels} summaryFor={summaryFor} /> : <p className="empty-copy">None of the selected models have complete, reference-blind runs for this dataset. Recorded results remain in the table above.</p>}</section>

      <section className="model-list"><div className="section-title"><h2>Configurations</h2></div>{visibleModels.map(model => <article key={model.id}><span>{String(models.indexOf(model) + 1).padStart(2, '0')}</span><div><h3>{model.name}</h3><strong>{model.runtime}</strong></div><p>{model.prompt_mode}</p></article>)}</section>
      </>}
    </div>
  );
}
